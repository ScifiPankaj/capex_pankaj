...
import { mastersApi } from '../../../reducers/features/masters/mastersApi';
...
const CapexMastersSetup = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await mutateMaster({ url, method, body: JSON.stringify(submit) }).unwrap();
      closeModal();
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 2500);
    } catch (e) {
      setErrorMessage(
        `Error: ${e?.data || e?.error || e?.message || 'Unknown error'}`
      );
    }
  };
  ...
  const handleDelete = async (masterKey, item) => {
    ...
    try {
      ...
      alert(
        `Error deleting item: ${e?.data || e?.error || e?.message || 'Unknown error'}`
      );
    }
  };
  ...
  return (
    ...
    >
      {/* Header inside main layout content */}
      <div
        style={{
          padding: '12px 20px',
          background: 'white',
      ...
      {/* Content Grid – fits inside parent layout */}
      <div
        style={{
          flex: 1,
          padding: '4px',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      >
                        {items.length} {items.length === 1 ? 'entry' : 'entries'} • ID:{' '}
                        {config.idField}
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                ...
                >
                  {React.createElement(MASTER_CONFIGS[currentMaster].icon, {
                    size: 24,
                    color: 'white',
                    strokeWidth: 2.5,
                  })}
                ...
                <div>
                  ...
                  >
                    ...
                    {MASTER_CONFIGS[currentMaster].title}
                  </h2>
                  <p
                    style={{
                      fontSize: '14px',
                  ...
                  >
                    {editingItem
                      ? `Updating record with ${MASTER_CONFIGS[currentMaster].idField}`
                      ...
                      <label
                        ...
                        onMouseEnter={(e) => {
                          e.currentTarget.style.borderColor =
                            MASTER_CONFIGS[currentMaster].color;
                          e.currentTarget.style.background = 'white';
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.borderColor = '#e5e7eb';
